...
public class BillServiceTest {
    ...
    @Test
    public void saveBillShouldSaveAllBillPayments() {
        Person person = Person.builder().firstName("Ingmar").lastName("Liibert").email("ingmar@liibert").build();
        List<BillPayment> billPayments = Arrays.asList(BillPayment.builder().id(1L).sum(BigDecimal.TEN).person(person).build());
